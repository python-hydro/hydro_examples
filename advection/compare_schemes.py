...
def run_weno(order=2, nx=32, optimized=False):
...
def weno_string(order=2, nx=32, optimized=False):
    return f"run_weno(order={order}, nx={nx}, optimized={optimized})"

def dg_string(m=3, nx=8, limiter=None):
    return f"run_dg(m={m}, nx={nx}, limiter='{limiter}')"

def time_weno(order=2, nx=32, optimized=False):
    return timeit.timeit(weno_string(order, nx, optimized),
...
def errs_weno(order=2, nx=32, optimized=False):
    g = advection.Grid1d(nx, ng=order+1, xmin=0, xmax=1)
    s = weno.WENOSimulation(g, 1, C=0.5, weno_order=order)
    s.init_cond("sine")
    a_init = g.a.copy()
...
weno_orders = [3, 5, 7]
weno_N = [8, 12, 16, 24, 32, 54, 64]
#weno_orders = [3]
#weno_N = [24, 32, 54]
weno_times = numpy.zeros((len(weno_orders), len(weno_N)))
weno_opt_times = numpy.zeros((len(weno_orders), len(weno_N)))
weno_errs = numpy.zeros_like(weno_times)
weno_opt_errs = numpy.zeros_like(weno_opt_times)
...
#        weno_memory[i_o, i_n] = max(memory_usage((run_weno, (order, nx))))
        weno_opt_errs[i_o, i_n] = errs_weno(order, nx, optimized=True)
        weno_opt_times[i_o, i_n] = time_weno(order, nx, optimized=True)
#        weno_opt_memory[i_o, i_n] = max(memory_usage((run_weno, 
#                                                     (order, nx, optimized))))

dg_ms = [2, 3, 4]
dg_N = 2**numpy.array(range(3, 9))
#dg_ms = [2]
#dg_N = 2**numpy.array(range(3, 6))
dg_nolimit_times = numpy.zeros((len(dg_ms), len(dg_N)))
dg_moment_times = numpy.zeros((len(dg_ms), len(dg_N)))
dg_nolimit_errs = numpy.zeros_like(dg_nolimit_times)
...
for i_m, m in enumerate(dg_ms):
    for i_n, nx in enumerate(dg_N):
        ...
        dg_nolimit_errs[i_m, i_n] = errs_dg(m, nx, None)
        dg_nolimit_times[i_m, i_n] = time_dg(m, nx, None)
#        dg_nolimit_memory[i_m, i_n] = max(memory_usage((run_dg,
#                                                       (m, nx, None))))
...
for i_o, order in enumerate(weno_orders):
    ...
    ax.loglog(weno_opt_times[i_o, :], weno_opt_errs[i_o, :],
              f'{colors[i_o]}o--', label=f'WENO, optimized, r={order}')
colors = "gyc"
for i_m, m in enumerate(dg_ms):
    ax.loglog(dg_nolimit_times[i_m, :], dg_nolimit_errs[i_m, :], 
              f'{colors[i_m]}s-.', label=rf'DG, no limiter, $m={m}$')
    ax.loglog(dg_moment_times[i_m, :], dg_moment_errs[i_m, :],
              f'{colors[i_m]}^:', label=rf'DG, moment limiter, $m={m}$')
